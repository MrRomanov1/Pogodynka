...
public class CurrentWeather {
    public static void setCurrentWeatherLabels(VBox weatherVbox, WeatherData weatherData) {
        ...
        Label descriptionLabel = new Label(AdditionalMethods.capitalize(weatherData.getCurrent().getWeather().
                get(0).getDescription()));

        Label updateTimeLabel = new Label("Zaktualizowano o godzinie: " + DateConverter.dateConverter(weatherData.
                getCurrent().getDt()));
    ...
    public static void setCurrentWeatherFirstBox(HBox firstBox, WeatherData weatherData) {
        ...
        Label feelsLikeLabel = new Label("Temperatura odczuwalna: " + String.format("%3.1f",
                weatherData.getCurrent().getFeels_like()) + "°C");

        Label windLabel = new Label("Wiatr: " + String.format("%4.1f",
                weatherData.getCurrent().getWind_speed()) + " m/s");
        SVGPath windDirection = new SVGPath();
        windDirection.setContent("M15,6H8V3c0-0.484-0.375-1-1-1C6.539,2,6.305,2.287,6,2.54L0.625,7C0.242,7.313,0,7.555," +
        ...
        Label visibilityLabel = new Label("Widoczność: " + weatherData.getCurrent().getVisibility() / 1000 + " km");
    ...
    public static void setCurrentWeatherSecondBox(HBox secondBox, WeatherData weatherData) {
        ...
        Label dewPoint = new Label ("Temperatura punktu rosy: " + String.format("%3.1f", weatherData.getCurrent().
                getDew_point()) + "°C");
